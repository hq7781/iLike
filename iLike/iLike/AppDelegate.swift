...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        self.setAppStartKeyWindow()
        
        // Twitter Kit SDK
        Twitter.sharedInstance().start(withConsumerKey:TwitterConsumerKEY,
    ...
    func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool {
        if Twitters.shared.application(app, open: url, options: options) {
            print("Twitter Login callback \(options)")
            UIUtil.toast(" Twitter Login successed")
            return true
        }
        return false
    }
    ...
    private func getRootViewController() -> UIViewController {
        if true /*!UserDefaultsHelper.notFirstSetup*/ {
            return LoginViewController()
        } else if true /*!UserDefaultsHelper.notFirstSetup*/ {
